...
export default function Register({
    ...
    `w-full border rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 transition-all ${
      hasErr
        ? "border-rose-500/60 focus:border-rose-500 focus:ring-rose-500/20 " +
          (D
            ? "bg-[#1a1d2e] text-white placeholder-slate-500"
            : "bg-slate-50 text-slate-800 placeholder-slate-400")
        : D
          ? "bg-[#1a1d2e] border-white/10 text-white placeholder-slate-500 focus:border-violet-500 focus:ring-violet-500/20"
  ...
  function validate() {
    ...
    if (form.password.length < 8)
      ...
      e.password = "Password needs an uppercase letter";
    else if (!/[0-9]/.test(form.password))
      e.password = "Password needs a number";
    ...
    >
      ...
      <PageNav
        ...
        rightSlot={
          <button
            onClick={onGoLogin}
            className={`hidden sm:block text-xs font-semibold px-3 py-1.5 rounded-lg border transition-colors ${D ? "border-white/10 text-slate-300 hover:bg-white/10 hover:text-white" : "border-slate-200 text-slate-600 hover:bg-slate-50 hover:text-slate-800"}`}
          >
            Sign in
          </button>
        }
      ...
      <div className="min-h-screen flex items-center justify-center px-4 pt-16">
        <div className="w-full max-w-sm py-12">
          <div className="flex flex-col items-center mb-8">
            ...
            <p className={`text-sm mt-1 ${txtMut}`}>
          ...
          <div
            ...
            style={{ background: D ? "#111320" : "#fff", border: cardBrd }}
          >
            {apiError && (
              <div
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label
                  className={`text-xs font-semibold mb-1.5 block ${labelCl}`}
                >
                  Full name
                </label>
                <input
                  ...
                  <div className="mt-2">
                    <div className="flex gap-1 mb-1">
                      {[1, 2, 3, 4].map((i) => (
                        <div
                          key={i}
                          className={`h-1 flex-1 rounded-full transition-all duration-300 ${i <= strength ? strengthColor : D ? "bg-white/10" : "bg-slate-200"}`}
                        />
                      ))}
                    </div>
              ...
              <div>
                ...
                />
                {errors.confirm && (
                  <p className="text-xs text-rose-400 mt-1">{errors.confirm}</p>
                )}
              </div>
              ...
              <div className="flex items-start gap-2.5">
                <input
                  ...
                  id="terms"
                  required
                  className="w-4 h-4 mt-0.5 rounded accent-violet-600 cursor-pointer flex-shrink-0"
                />
                <label
                  htmlFor="terms"
                  className={`text-xs cursor-pointer ${txtMut}`}
                >
                  I agree to the{" "}
            ...
            <div className="flex items-center gap-3 my-5">
              <div
                ...
                style={{ background: D ? "rgba(255,255,255,0.08)" : "#e2e8f0" }}
              />
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                <button
                  key={i}
                  className={`flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-medium transition-colors ${D ? "text-slate-300 hover:text-white" : "text-slate-600 hover:text-slate-800"}`}
                  style={{
                    border: D
