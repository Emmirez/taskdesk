...
function Toggle({ on, onToggle, dark }) {
  return (
    ...
    >
      <span
        className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-all ${on ? "left-[20px]" : "left-0.5"}`}
      />
    </button>
  );
}
...
function Section({ title, children, dark, T }) {
  return (
    <div className={`border rounded-2xl overflow-hidden ${T.cardCls}`}>
      ...
      <div
        className="divide-y"
        style={{ borderColor: dark ? "rgba(255,255,255,0.06)" : "#f1f5f9" }}
      >
        {children}
      </div>
    </div>
...
export default function Settings({ user }) {
  ...
  async function savePassword(e) {
    ...
    if (!/[A-Z]/.test(pwForm.next)) {
      setPwMsg({ text: "Password needs an uppercase letter", error: true });
      return;
    }
    if (!/[0-9]/.test(pwForm.next)) {
      setPwMsg({ text: "Password needs a number", error: true });
      return;
    }
  ...
  return (
    <div className="p-4 sm:p-6 max-w-2xl mx-auto w-full space-y-5">
      ...
      <Section title="Profile" dark={dark} T={T}>
        ...
        {nameMsg && (
          <div
            className="mx-5 mb-4 px-4 py-2 rounded-lg text-xs font-medium"
            style={{
              background: nameMsg.includes("!")
        ...
        <Row
          ...
          control={
            <span
              className={`text-xs px-2 py-1 rounded-lg ${dark ? "bg-white/8 text-slate-400" : "bg-slate-100 text-slate-500"}`}
            >
              Read only
            </span>
          }
      ...
      <Section title="Change Password" dark={dark} T={T}>
        <form onSubmit={savePassword} className="px-5 py-5 space-y-3">
          ...
          <div className="flex items-center justify-between pt-1">
            <label
              className={`flex items-center gap-2 text-xs cursor-pointer ${T.txtMut}`}
            >
              <input
                type="checkbox"
                checked={showPw}
      ...
      <Section title="Appearance" dark={dark} T={T}>
        <Row
          ...
          T={T}
          control={
            <Toggle on={dark} onToggle={() => setDark((d) => !d)} dark={dark} />
          }
        />
        <Row
          label="Compact View"
          desc="Reduce spacing in task lists"
          T={T}
          control={
            <Toggle
              on={compact}
      ...
      <Section title="Notifications" dark={dark} T={T}>
        ...
        <Row
          label="Email Digests"
          desc="Daily task summary by email"
          T={T}
          control={
            <Toggle
              on={emailD}
              onToggle={() => setEmailD((e) => !e)}
              dark={dark}
            />
          }
        />
